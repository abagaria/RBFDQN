...
MODEL_DIR = os.path.join(os.path.expanduser("~"), 'git-repos/RBFDQN/tasks/point_maze/environments/assets')
...
class MazeEnv(gym.Env):
  ...
  def __init__(
    ...
    if model_cls is None:
      raise Warning("MODEL_CLASS unspecified!")
  ...
  def _get_obs(self):
    ...
    if self._observe_blocks:
      ...
      wrapped_obs = np.concatenate([wrapped_obs[:3]] + additional_obs +
                                   [wrapped_obs[3:]])
    wrapped_obs = np.hstack([wrapped_obs[:2]] + [wrapped_obs[2:]])
